...
<?php
            ...
            if ($tag->basetype == 'repeater') {
                if ($contact_form->prop('repeater')) {
                    ...
                    foreach ($repeaters as $index => $repeater) {
                        ...
                        $_value_index = 0;
                        $indexes = array();
                        foreach ($this->posted_data as $key => $r) {
                            foreach ($repeater_tags as $_tag) {
                                if (preg_match('/^' . $_tag['name'] . '-[0-9]+-[0-9]+$/', $key)) {
                                    $indexes[] = str_replace($_tag['name'], "", $key);
                                }
                            }
                        }
                        ...
                        foreach ($indexes as $oindex) {
                            foreach ($repeater_tags as $_tag) {
                                ...
                                if ($_tag->type == 'file') {
                                    ...
                                    $values = preg_grep('/^' . $name . '\-' . $index . '\-' . $_value_index . '/', array_keys($_FILES));
                                    foreach ($values as $_name) {
                                        $names = $this->parseFiles($_name);
                                        foreach ($names as $fname) {
                                            //if the repeater uploaded the file, the attachment_id will be set, otherwise we will try to upload it again
                                            $attachment_id = $_FILES[$fname]['attachment_id'] ?? media_handle_upload($fname, 0);
                                            if (!is_wp_error($attachment_id)) {
                                                $fields['repeater-' . $index][$_value_index][$name] = wp_get_attachment_url($attachment_id);
                                            }
                                        }
                                    }
                                ...
                                $values = preg_grep('/^' . $name . '\-' . $index . '\-' . $_value_index . '/', array_keys($this->posted_data));
                ...
                if (!empty($this->posted_data[$tag->name])) {
                    if (!is_array($this->posted_data[$tag->name])) {
                        $fields[$tag->name] = (string)$this->posted_data[$tag->name];
                    } else {
                        $fields[$tag->name] = implode(', ', $this->posted_data[$tag->name]);
                    }
                } else {
                    $fields[$tag->name] = '';
                }
    ...
    private function parseFiles($name)
    {
        $fname = uniqid();
        $files = [];
        $value = $_FILES[$name];
